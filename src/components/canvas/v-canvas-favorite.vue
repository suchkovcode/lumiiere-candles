<template>
   <div class="favorite" :class="{ active: activeState }">
      <v-canvas-header @closeCanvas="emitCloseCanvas"> Избранное: </v-canvas-header>
      <div v-if="cards.length > 0" class="favorite__list">
         <v-canvas-favorite-item v-for="item in cards" :key="item.id" :favoriteItem="item" @delFavorite="delFavorite(item.id)" />
      </div>
